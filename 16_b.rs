...
fn main() -> io::Result<()> {
  ...
  let in_h = "C200B40A82";
  let in_i = "04005AC33890";
  let in_j = "880086C3E88112";
  let in_k = "CE00C43D881120";
  let in_l = "D8005AC2A8F0";
  let in_m = "F600BC2D8F";
  let in_n = "9C005AC2F8F0";
  let in_o = "9C0141080250320F1802104A08";
  ...
  while (bin_input.len() - idx) > 11 {
    let (new_val, new_vsum, new_idx) = process(val, vsum, &bin_input, idx);
    vsum = new_vsum;
    idx = new_idx;
    val = new_val;
...
fn process(mut val: i64, mut vsum: i32, bin_input: &String, mut idx: usize) -> (i64, i32, usize) {
  ...
  if typeid == "100" {
    ...
    // MAX
      ...
      val = max;
      //println!("MAX result: {}", val);
    // GREATER THAN
    } else if typeid == "101" {
      let mut r2 = results.pop();
      let mut r1 = results.pop();
      //println!("GT");
      let mut res = 0;
      if r1 > r2 {res = 1;}
      val = res;
      //println!("GT result: {}", val);
    // LESS THAN
    } else if typeid == "110" {
      let mut r2 = results.pop();
      let mut r1 = results.pop();
