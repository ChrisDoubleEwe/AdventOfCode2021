...
fn main() -> io::Result<()> {
  ...
  for from in 0..spaces.len() {
    for to in 0..spaces.len() {
      ...
      if from_y == 4 {
        path.push(coord_to_space(from_x as i32, (from_y-1) as i32, spaces.clone()));
        path.push(coord_to_space(from_x as i32, (from_y-2) as i32, spaces.clone()));
      }
      if from_y == 5 {
        path.push(coord_to_space(from_x as i32, (from_y-1) as i32, spaces.clone()));
        path.push(coord_to_space(from_x as i32, (from_y-2) as i32, spaces.clone()));
        path.push(coord_to_space(from_x as i32, (from_y-3) as i32, spaces.clone()));
      }
      ...
      // Down
      if to_y == 5 {
        path.push(coord_to_space(to_x as i32, (to_y-3) as i32, spaces.clone()));
        path.push(coord_to_space(to_x as i32, (to_y-2) as i32, spaces.clone()));
...
fn we_have_seen(mut thing: String, my_score: i32) -> bool {
  let mut seen = false;
  unsafe {
  for x in all_history.clone() {
    if x.0 == thing {
      if my_score >= x.1 {
        seen = true;
        //println!("Seen, with a lower score"); 
      } else {
        //println!("Seen, with a higher score");
      }
    } else {
    }
  }
...
fn make_move(paths: Vec<Vec<Vec<i32>>>, mut score: i32, mut history: Vec<String>, mut map: Vec<char>, steps: Vec<Vec<i32>>) -> (i32, Vec<String>, Vec<char>)  {
  ...
  //println!("Making move: {} : {}", map_string.clone(), score);

  

  if map_string == ".......+AAAA+BBBB+CCCC+DDDD" {
  ...
  unsafe {
  if we_have_seen(map_string.clone(), score) {
  ...
  //     If piece in correct silo, lower berth, forget it: we're done
    ...
    if (map[piece_num] == 'A' && piece_num == 19 ) ||
       (map[piece_num] == 'B' && piece_num == 20 ) ||
       (map[piece_num] == 'C' && piece_num == 21 ) ||
       (map[piece_num] == 'D' && piece_num == 22 ) {
         continue;
    }

  //     If piece in correct silo, upper berth, and lower berth is OK, forget it: we're done
    if (map[piece_num] == 'A' && piece_num == 15 && map[19] == 'A') ||
       (map[piece_num] == 'B' && piece_num == 16 && map[20] == 'B') ||
       (map[piece_num] == 'C' && piece_num == 17 && map[21] == 'C') ||
       (map[piece_num] == 'D' && piece_num == 18 && map[22] == 'D') {
         continue;
    }

    if (map[piece_num] == 'A' && piece_num == 11 && map[15] == 'A' && map[19] == 'A') ||
       (map[piece_num] == 'B' && piece_num == 12 && map[16] == 'B' && map[20] == 'B') ||
       (map[piece_num] == 'C' && piece_num == 13 && map[17] == 'C' && map[21] == 'C') ||
       (map[piece_num] == 'D' && piece_num == 14 && map[18] == 'D' && map[22] == 'D') {
         continue;
    }    
    if (map[piece_num] == 'A' && piece_num == 7 && map[11] == 'A' && map[15] == 'A' && map[19] == 'A') ||
       (map[piece_num] == 'B' && piece_num == 8 && map[12] == 'B' && map[16] == 'B' && map[20] == 'B') ||
       (map[piece_num] == 'C' && piece_num == 9 && map[13] == 'C' && map[17] == 'C' && map[21] == 'C') ||
       (map[piece_num] == 'D' && piece_num == 10 && map[14] == 'D' && map[18] == 'D' && map[22] == 'D') {
         continue;
    }
  ...
  //     If piece is anywhere, and correct silo lower berth is empty & accessible, move.
    if (map[piece_num] == 'A' && map[19] == '.' && path_is_clear(piece_num as i32, 19, map.clone(), paths.clone())) {
      moved = 1;
      map[piece_num] = '.';
      map[19] = 'A';
      score += steps[piece_num][19];
      let (score, history, map) = make_move(paths.clone(), score, history.clone(), map.clone(), steps.clone());
    }
    if (map[piece_num] == 'B' && map[20] == '.' && path_is_clear(piece_num as i32, 20, map.clone(), paths.clone())) {
      moved = 1;
      map[piece_num] = '.';
      map[20] = 'B';
      score += 10*steps[piece_num][20];
      let (score, history, map) = make_move(paths.clone(), score, history.clone(), map.clone(), steps.clone());
    }
    if (map[piece_num] == 'C' && map[21] == '.' && path_is_clear(piece_num as i32, 21, map.clone(), paths.clone())) {
      moved = 1;
      map[piece_num] = '.';
      map[21] = 'C';
      score += 100*steps[piece_num][21];
      let (score, history, map) = make_move(paths.clone(), score, history.clone(), map.clone(), steps.clone());
    }
    if (map[piece_num] == 'D' && map[22] == '.' && path_is_clear(piece_num as i32, 22, map.clone(), paths.clone())) {
      moved = 1;
      map[piece_num] = '.';
      map[22] = 'D';
      score += 1000*steps[piece_num][22];
  ...
  //     If piece is anywhere, and correct silo upper berth is E&A, and lower silo has correct occupant, move
    if (map[piece_num] == 'A' && map[19] == 'A' && map[15] == '.' && path_is_clear(piece_num as i32, 15, map.clone(), paths.clone())) {
      moved = 1;
      map[piece_num] = '.';
      map[15] = 'A';
      score += steps[piece_num][15];
      let (score, history, map) = make_move(paths.clone(), score, history.clone(), map.clone(), steps.clone());
    }
    if (map[piece_num] == 'B' && map[20] == 'B' && map[16] == '.' && path_is_clear(piece_num as i32, 16, map.clone(), paths.clone())) {
      moved = 1;
      map[piece_num] = '.';
      map[16] = 'B';
      score += 10*steps[piece_num][16];
      let (score, history, map) = make_move(paths.clone(), score, history.clone(), map.clone(), steps.clone());
    }
    if (map[piece_num] == 'C' && map[21] == 'C' && map[17] == '.' && path_is_clear(piece_num as i32, 17, map.clone(), paths.clone())) {
      moved = 1;
      map[piece_num] = '.';
      map[17] = 'C';
      score += 100*steps[piece_num][17];
      let (score, history, map) = make_move(paths.clone(), score, history.clone(), map.clone(), steps.clone());
    }
    if (map[piece_num] == 'D' && map[22] == 'D' && map[18] == '.' && path_is_clear(piece_num as i32, 18, map.clone(), paths.clone())) {
      moved = 1;
      map[piece_num] = '.';
      map[18] = 'D';
      score += 1000*steps[piece_num][18];
    ...
    if (map[piece_num] == 'A' && map[19] == 'A' && map[15] == 'A' && map[11] == '.' && path_is_clear(piece_num as i32, 11, map.clone(), paths.clone())) {
      moved = 1;
      map[piece_num] = '.';
      map[11] = 'A';
    ...
    if (map[piece_num] == 'B' && map[20] == 'B' && map[16] == 'B' && map[12] == '.' && path_is_clear(piece_num as i32, 12, map.clone(), paths.clone())) {
      moved = 1;
      map[piece_num] = '.';
      map[12] = 'B';
    ...
    if (map[piece_num] == 'C' && map[21] == 'C' && map[17] == 'C' && map[13] == '.' && path_is_clear(piece_num as i32, 13, map.clone(), paths.clone())) {
      moved = 1;
      map[piece_num] = '.';
      map[13] = 'C';
    ...
    if (map[piece_num] == 'D' && map[22] == 'D' && map[18] == 'D' && map[14] == '.' && path_is_clear(piece_num as i32, 14, map.clone(), paths.clone())) {
      moved = 1;
      map[piece_num] = '.';
      map[14] = 'D';
    ...
    if (map[piece_num] == 'A' && map[11] == 'A' && map[19] == 'A' && map[15] == 'A' && map[7] == '.' && path_is_clear(piece_num as i32, 7, map.clone(), paths.clone())) {
      moved = 1;
      map[piece_num] = '.';
      map[7] = 'A';
    ...
    if (map[piece_num] == 'B' && map[12] == 'B' && map[20] == 'B' && map[16] == 'B' && map[8] == '.' && path_is_clear(piece_num as i32, 8, map.clone(), paths.clone())) {
      moved = 1;
      map[piece_num] = '.';
      map[8] = 'B';
    ...
    if (map[piece_num] == 'C' && map[13] == 'C' && map[21] == 'C' && map[17] == 'C' && map[9] == '.' && path_is_clear(piece_num as i32, 9, map.clone(), paths.clone())) {
      moved = 1;
      map[piece_num] = '.';
      map[9] = 'C';
    ...
    if (map[piece_num] == 'D' && map[14] == 'D' && map[22] == 'D' && map[18] == 'D' && map[10] == '.' && path_is_clear(piece_num as i32, 10, map.clone(), paths.clone())) {
      moved = 1;
      map[piece_num] = '.';
      map[10] = 'D';
  ...
  unsafe {
  if ((score+add_steps) as i64) >= low_score {
    return (score, history.clone(), map.clone())
  }
  }
